...
Y_full=zeros(7,size(t,2));

%For test 5
Y_full(1,1:m_max+1)=exp(12.2057445) - 1;
Y_full(2,1:m_max+1)=exp(2.049243007) - 1;
Y_full(3,1:m_max+1)=exp(0.602479062) - 1;
Y_full(4,1:m_max+1)=0;
Y_full(5,1:m_max+1)=exp(9.068502404) - 1;
Y_full(6,1:m_max+1)=exp(9.541069952) - 1;
Y_full(7,m_max:m_max+1)=exp(9.358962044) - 1 ;

% max+2 - min index: t>0 
start_index=m_max+2;

k = start_index;
delays=[Y_full(:,k-m(1)),Y_full(:,k-m(2)),Y_full(:,k-m(3)),Y_full(:,k-m(4))];
Y_int = Y_full(:,k-m(1):k-1); 
[Y_full(:,k),~]=Step(Y_int, delays,coef,delta,err,scheme);
...
tic
nns=0; 
for k=start_index+1:size(t,2) 
    delays=[Y_full(:,k-m(1)),Y_full(:,k-m(2)),Y_full(:,k-m(3)),Y_full(:,k-m(4))];
    Y_int = Y_full(:,k-m(1):k-1); 
    [Y_full(:,k),nnsk]=Step(Y_int, delays,coef,delta,err,scheme);
    nns=nns+nnsk;
    if (mod(k-start_index+1,inc)==0) 
        fprintf(1,'t=%g, nns=%g\n',t(k),nns);
...
% 
norm(U_full(:,1:end)-Y_full)/norm(Y_full)
%
